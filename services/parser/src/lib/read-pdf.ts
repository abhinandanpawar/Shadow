...
// Custom type definition for PdfjsTextItem since we are not using the package
type PdfjsTextItem = {
  str: string;
  dir: string;
  transform: number[];
 ...
 *     const fileBuffer = e.target.files[0].arrayBuffer();
 *     const textItems = await readPdf(fileBuffer);
 * }
 */
export const readPdf = async (fileBuffer: Buffer): Promise<TextItems> => {
  const pdfFile = await pdfjs.getDocument({ data: fileBuffer }).promise;
