...
import { useResumeStore, ResumePreview } from '@/store/resume';

const Resume: React.FC = () => {
    const resumePreview = useResumeStore((state) => state.improvedResult?.data.resume_preview);

    if (!resumePreview) {
        return (
            <div className="bg-gray-900/80 p-6 rounded-lg shadow-xl text-gray-100 border border-gray-800/50 h-full flex items-center justify-center">
                <p className="text-gray-400">Resume preview will appear here once analysis is complete.</p>
            </div>
        );
    }

	const { personalInfo, summary, experience, education, skills } = resumePreview;
	...
	return (
		...
		<div className="font-mono bg-gray-950 text-gray-300 p-4 shadow-lg rounded-lg max-w-4xl mx-auto border border-gray-600 h-full overflow-auto">
